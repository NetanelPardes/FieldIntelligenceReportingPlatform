...
# הנתיב לתיקייה הראשית של הפרויקט
...
def send_reports(
    ...
    if undelivered_messages > 0:
        raise RuntimeError(
            f"{undelivered_messages} messages were not delivered"
        )

    logger.info(
        "Successfully sent %s reports to topic '%s'",
        len(reports),
        topic_name
    )
...
def main():
    ...
    reports_file_path = os.getenv(
        "REPORTS_FILE_PATH"
    )

    if not bootstrap_servers:
        raise ValueError(
            "KAFKA_BOOTSTRAP_SERVERS is missing from .env"
        )

    if not topic_name:
        raise ValueError(
            "REPORTS_TOPIC is missing from .env"
        )

    if not reports_file_path:
        raise ValueError(
            "REPORTS_FILE_PATH is missing from .env"
        )
    ...
    full_file_path = (
        root_directory / reports_file_path
    )

    reports = load_reports(full_file_path)

    logger.info(
        "Successfully loaded %s reports",
        len(reports)
    )
...
if __name__ == "__main__":
    try:
        ...
        logger.error(
            "Reports file contains invalid JSON: %s",
            error
        )
